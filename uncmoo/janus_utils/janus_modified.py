...
from janus.crossover import crossover_smiles
from janus.mutate import mutate_smiles
from janus.network import create_and_train_network, obtain_model_pred
from janus.utils import sanitize_smiles, get_fp_scores
from janus.fragment import form_fragments
...
class ModifiedJanus():
    ...
    def __init__(
        ...
        init_fitness = []
        if self.batch_pred:
            scores = self.fitness_function(init_smiles)
            init_fitness = [score for score in scores]
        else:
    ...
    def run(self):
        ...
        for gen_ in range(self.generations):
            ...
            if self.batch_pred:
                ...
                self.fitness = [score for score in scores]
                for smi, score in zip(self.population, self.fitness):
                    if self.smiles_collector.get(smi):
                        self.smiles_collector[smi][1] += 1
                    else:
                        self.smiles_collector[smi] = [score, 1]
            else:
            ...
            self.fitness_loc = []
            if self.batch_pred:
                scores = self.fitness_function(self.population_loc)
                for smi, score in zip(self.population_loc, scores):
                    self.fitness_loc.append(score)
                    self.smiles_collector[smi] = [score, 1]
                pass
            else:
    ...
    def get_good_bad_smiles(fitness, population, generation_size):
        ...
        try:
            ...
            # xx = (F_50_val - fitness) / (F_50_val - F_25_val)
            # prob_ = 3.0**(-xx) / ( 1 + (3.0**(-xx)) )
